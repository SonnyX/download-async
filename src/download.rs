...
impl Downloader {
    ...
    pub async fn download<T: HttpBody + Send + 'static>(self, request: crate::Request<T>, mut progress: impl Progress, mut to: impl Write, socket_addrs: crate::dns::SocketAddrs, https_only: bool) -> Result<(), Box<dyn Error>> where T::Data: Send, T::Error: Into<Box<dyn Error + Send + Sync>> {
